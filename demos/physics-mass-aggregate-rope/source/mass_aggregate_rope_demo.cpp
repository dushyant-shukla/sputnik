#include "mass_aggregate_rope_demo.h"
...
MassAggregateRopeDemoLayer::MassAggregateRopeDemoLayer(const std::string& name)
    : core::Layer{name}
...
void MassAggregateRopeDemoLayer::OnAttach()
{
    ...
    const u32 total_particles = (u32)(kRopeGridResolution.x * kRopeGridResolution.y * kRopeGridResolution.z);
    ...
    for(size_t i = 0; i < m_particles.size(); i++)
    {
        if(i == 1) // second particle in the rope
        {
            ...
            m_cable_start             = std::make_shared<AnchoredParticleCable>();
            m_cable_start->m_particle = m_particles[i];
            m_cable_start->m_anchor   = m_particles[0]->getPosition(); // first particle is an anchor
            // cable->m_max_length                          = kParticleRadius * 12.5f;
            m_cable_start->m_max_length  = kCableLength;
            m_cable_start->m_restitution = kRestitution;
            m_anchored_cables.push_back(m_cable_start);
            m_physics_system->addContactGenerator(m_cable_start);
        }
        //else if(i == kRopeGridResolution.x - 2) // penultimate particle in the rope
        //{
        //    m_particles[i + 1]->setAcceleration({0.0f});
        //    m_cable_end             = std::make_shared<AnchoredParticleCable>();
        //    m_cable_end->m_particle = m_particles[i];
        //    m_cable_end->m_anchor   = m_particles[i + 1]->getPosition(); // last particle is an anchor
        //    // cable->m_max_length  = kParticleRadius * 12.5f;
        //    m_cable_end->m_max_length  = kCableLength;
        //    m_cable_end->m_restitution = kRestitution;
        //    m_anchored_cables.push_back(m_cable_end);
        //    m_physics_system->addContactGenerator(m_cable_end);
        //}
        //else if(i > 1 && i < kRopeGridResolution.x - 2) // all other particles in the rope
        else if(i > 1 && i < kRopeGridResolution.x - 1) // all other particles in the rope
        {
...
void MassAggregateRopeDemoLayer::OnUpdate(const core::TimeStep& time_step)
{
    ...
    m_cable_start->m_anchor = m_particles[0]->getPosition();
    //m_cable_end->m_anchor   = m_particles[kRopeGridResolution.x - 1]->getPosition();

    //m_cables[0]->m_particles[0] = m_particles[0];
    ...
    else
    {
        ...
        m_render_system->drawDebugLines(vertices, {0.7f, 0.7f, 0.7f}, 10.0f);
        vertices.clear();
    }
}
...
void MassAggregateRopeDemoLayer::OnUpdateUI(const core::TimeStep& time_step)
{
    ...
    {
        auto particle = m_cables[m_cables.size() - 1]->m_particles[1];
