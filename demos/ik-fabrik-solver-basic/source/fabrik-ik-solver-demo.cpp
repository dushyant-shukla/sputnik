...
void FabrikIKDemo::OnAttach()
{
    ...
    m_solver[3].position = {0.0f, 7.5f, 0.5f};
    m_solver[3].rotation = AngleAxis(90.0f * Constants::DEG_TO_RAD, {0.0f, 7.5f, 0.0f});
    m_solver[4].position = {0.0f, 7.5f, 0.5f};
    m_solver[5].position = {0.0f, 7.5f, 0.5f};
    ...
    EditorCamera::GetInstance()->SetPosition({5.0f, 10.5f, 50.0f});
...
void FabrikIKDemo::OnUpdateUI(const core::TimeStep& time_step)
{
    ...
    const auto& m_projection = graphics::api::EditorCamera::GetInstance()->GetCameraPerspective();
    const auto& m_view       = graphics::api::EditorCamera::GetInstance()->GetCameraView();
    const auto& m_mvp        = m_projection * m_view;
