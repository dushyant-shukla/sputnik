...
vec3 Random::randomVector(real scale)
{
    return vec3(randomBinomial(scale), randomBinomial(scale), randomBinomial(scale));
}

vec3 Random::randomXZVector(real scale)
{
    return vec3(randomBinomial(scale), 0, randomBinomial(scale));
}

vec3 Random::randomVector(const vec3& scale)
{
    return vec3(randomBinomial(scale.x), randomBinomial(scale.y), randomBinomial(scale.z));
}

vec3 Random::randomVector(const vec3& min, const vec3& max)
{
    return vec3(randomReal(min.x, max.x), randomReal(min.y, max.y), randomReal(min.z, max.z));
}
}
